...
function AdminPanel({
 ...
 const [tForm, setTForm] = useState({ 
  title: "", 
  subject: "", 
  level: "Beginner", // Default value set hai
  duration: "" 
});
 ...
 // 1. Pehle ye ensure karein ki aapka useState aisa ho:


// 2. Updated handleTestSubmit function:
const handleTestSubmit = async (e) => {
  e.preventDefault();
  
  // Validation check
  if (!tForm.title || !tForm.subject || !tForm.level || !tForm.duration) { 
    alert("Please fill all fields, including Level."); 
    return; 
  }

  // Filter logic (Make sure 'subject' and 'level' exactly match your DB)
  const qIds = questions.filter((q) => 
    q.subject?.toUpperCase() === tForm.subject.toUpperCase() && 
    q.level === tForm.level
  ).map((q) => q.id);

  if (qIds.length === 0) { 
    alert("No questions found for this Subject and Level combination!"); 
    return; 
  }
            ...
            <form onSubmit={handleTestSubmit} className="space-y-5">
              ...
              <div>
                ...
                <input
                  ...
                  placeholder="e.g. Weekly Aptitude Challenge"
                  className="w-full border-2 border-slate-100 rounded-2xl p-4 focus:border-blue-500 transition font-bold"
                />
              </div>

              {/* Grid: Subject, Duration, Level */}
              <div className="grid grid-cols-3 gap-4">
                <div>
                  <label className="text-xs font-black text-slate-400 uppercase ml-1 mb-1 block">Subject</label>
                  <input
                    value={tForm.subject}
                    onChange={(e) => setTForm({ ...tForm, subject: e.target.value.toUpperCase() })}
                    placeholder="HTML"
                    className="w-full border-2 border-slate-100 rounded-2xl p-4 focus:border-blue-500 transition font-bold"
                  />
                </div>
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setTForm({ ...tForm, duration: e.target.value })}
                    placeholder="30"
                    className="w-full border-2 border-slate-100 rounded-2xl p-4 focus:border-blue-500 transition font-bold"
                  />
                </div>
                <div>
                  ...
                  <select
                    value={tForm.level}
                    onChange={(e) => setTForm({ ...tForm, level: e.target.value })}
                    className="w-full border-2 border-slate-100 rounded-2xl p-4 focus:border-blue-500 transition font-bold bg-white text-slate-700"
                  >
              ...
              <button type="submit" disabled={loading || csvLoading} className={`w-full py-5 rounded-2xl font-black text-lg transition shadow-lg ${loading || csvLoading ? "bg-slate-300 cursor-not-allowed" : "bg-slate-900 hover:bg-slate-800 text-white"}`}>
